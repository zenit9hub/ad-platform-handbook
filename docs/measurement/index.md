---
title: "측정 · 검증 · 정합성"
description: "광고플랫폼에서 측정, 검증, 집계 차이, 정합성 조정 흐름을 설명한다."
---

# 측정 · 검증 · 정합성 카테고리 인덱스

## 카테고리 목적

이 카테고리는 광고플랫폼에서 `무엇을 어떻게 측정하고, 어떤 기준으로 검증하며, 집계 차이를 어떻게 맞추는가`를 다룬다. tracking, verification, discrepancy, reconciliation, 공급 경로 투명성을 현재 운영 관점에서 정리하는 것이 목표다.

## 현재 문서

1. [TrackingEvents, impression, click, quartile 이해](/measurement/tracking-events)
2. [Discrepancy와 Reconciliation 개요](/measurement/discrepancy-and-reconciliation)
3. [sellers.json과 schain 이해](/measurement/sellers-json-and-schain)

## 권장 읽기 순서

1. TrackingEvents, impression, click, quartile 이해
2. Discrepancy와 Reconciliation 개요
3. sellers.json과 schain 이해

## 다음 카테고리

- [채널과 포맷](/channels/)

## 이후 확장 카테고리

- [구현 패턴](/implementation/)
- [Trust · Web3 실험실](/lab/)
- [레퍼런스](/reference/)
