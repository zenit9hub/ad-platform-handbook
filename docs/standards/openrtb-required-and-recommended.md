...
|`user.data[]`|선택|높음|사용자 또는 오디언스 데이터 신호를 담는 Data 객체 배열이다.|
|`data.segment[]`|선택|높음|오디언스, 관심사, taxonomy 노드로 해석할 수 있는 segment 배열이다.|
|`segment.id`|선택|높음|매칭, 필터링, 리포팅에 쓰는 segment 식별자다.|
|`segment.name`|선택|중간|사람이 읽는 표시용 값이다. 로직 기준은 `id` 중심으로 두는 것이 안전하다.|
...
- `lvl1`, `lvl2`, `lvl3` 같은 내부 분류는 외부 표준 필드처럼 노출하지 않고, `user.data[].segment[]`와 taxonomy ID 체계로 해석하는 것이 안전하다.
